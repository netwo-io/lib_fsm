...
create function lib_fsm.state_machine_transition(state_machine__id$ uuid, event$ lib_fsm.event_identifier,
                                                 dry_run$ boolean default false) returns record as
$$
declare
    to_state record;
begin
    ...
    -- history management (see why we are doing it this way in _state_machine_store_event())
    perform lib_fsm._state_machine_store_event(abstract_state__id$ =>to_state.abstract_state__id,
                                               state_machine__id$ => state_machine__id$,
                                               event$ => event$);
...
create function lib_fsm._state_machine_store_event(abstract_state__id$ uuid,
                                                   state_machine__id$ uuid,
                                                   event$ lib_fsm.event_identifier) returns void as
$$
declare
    name$ varchar(30);
begin
    ...
    insert into lib_fsm.state_machine_event (state_machine__id, abstract_state__id, event, abstract_state_name,
                                             created_at)
